...
class MdgDeclarationStatusConnector @Inject()(override val http: HttpClient,
  ...
  override def getHeaders[A](date: DateTime, correlationId: UUID, vpr: ValidatedPayloadRequest[A]): Seq[(String, String)] = {
     super.getHeaders(date, correlationId, vpr) :+ "X-Conversation-ID" -> vpr.conversationId.toString
  }
}
...
trait MdgDeclarationConnector extends UsingCircuitBreaker {
  ...
  def send[A](xml: NodeSeq, date: DateTime, correlationId: UUID, apiVersion: ApiVersion)(implicit vpr: ValidatedPayloadRequest[A]): Future[HttpResponse] = {
    ...
    implicit val hc: HeaderCarrier = HeaderCarrier(extraHeaders = getHeaders(date, correlationId, vpr), authorization = Some(Authorization(bearerToken)))
  ...
  def getHeaders[A](date: DateTime, correlationId: UUID, vpr: ValidatedPayloadRequest[A]): Seq[(String, String)] = {
    Seq(
  ...
  def post[A](xml: NodeSeq, url: String)(implicit vpr: ValidatedPayloadRequest[A], hc: HeaderCarrier): Future[HttpResponse] = {

...
class XmlValidationServicePERFSpec extends UnitSpec with MockitoSugar with BeforeAndAfterEach {
  ...
  protected val configuration  = Configuration.from(Map(propertyName -> xsdLocations))
  protected val MockXml = mock[Node]




  def xmlValidationService: XmlValidationService = new XmlValidationService(configuration, schemaPropertyName = propertyName){}
...
//  }
  ...
  "XmlValidationService" should {
    ...
    "successfully validate a correct xml" in {
      ...
      println("starting test::::::::::: " + startTime)
    ...
    "fail the future with configured number of wrapped SAXExceptions when there are multiple errors in XML" in {
      when(configuration.getInt("xml.max-errors")).thenReturn(Some(2))

      val caught = intercept[SAXException] {
        await(xmlValidationService.validate(InvalidSubmissionXMLWith2Errors))
      }
      verify(configuration).getInt("xml.max-errors")
    ...
    "fail the future with system error when a configured maximum of xml errors is not a positive number" in {
      when(configuration.getInt("xml.max-errors")).thenReturn(Some(0))
